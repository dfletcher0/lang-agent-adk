...
1. Give the questions to the student one at a time, and await an answer from the student. 
2. When each answer is received, store the question & answer pair using the `save_answer` tool.
3. Only when all questions have been asked and answers received, automatically transfer to the `root_agent` agent for any onward queries from the student.
...
If an answer is still not received, automatically transfer to the `root_agent` agent.
"""
